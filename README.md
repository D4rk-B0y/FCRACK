...
- Running the `apt update` to install the pipenv.
- Add the executable Python path via `apt upgrade `.
- Running the `apt install python` to install required dependencies.
- Running the `git clone https://github.com/D4rk-B0y/FCRACK.git` to set the `github_user` environment variable.
- Running the `cd FCRACK` to set the `click enter` environment variable.
- Running the `python lm30_enc.py` command to run the bot!
